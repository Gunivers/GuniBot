...
import static net.gunivers.gunibot.custom_channel.CustomChannelCreator.*;
...
public class CustomChannelCommand extends Command
{
	...
	public void list(MessageCreateEvent event, List<String> args)
	{
		...
		for (DataTextChannel channel : getByGuild(Main.getBotInstance().getDataCenter().getDataGuild(event.getGuild().block())))
		{
	...
	public void add(MessageCreateEvent event, List<String> args)
	{
		...
		if (create(event.getMember().get(), Parser.parseCategory(String.valueOf(g.getCCActive()), g.getEntity()).blockFirst(), args.get(0),
			g.getEntity().getChannelById(Snowflake.of(g.getCCArchive())).ofType(Category.class).block(), Boolean.valueOf(args.get(1))))
		{
			DataTextChannel channel = getByOwner(event.getMember().get());
			event.getMessage().getChannel().flatMap(c -> c.createMessage(channel.getEntity().getMention() +" successfully created!")).subscribe();
		} else
	...
	public void del(MessageCreateEvent event)
	{
		...
		if (removeChannel(data))
			event.getMessage().getChannel().flatMap(c -> c.createMessage("The channel "+ data.getEntity().getName() +"was deleted")).subscribe();
		else
			event.getMessage().getChannel().flatMap(c -> c.createMessage("Could not delete your custom channel")).subscribe();
	}
	...
	public void renew(MessageCreateEvent event)
	{
		...
		if (renewChannel(data, Parser.parseCategory(""+ g.getCCActive(), g.getEntity()).blockFirst(), Parser.parseCategory(""+ g.getCCArchive(), g.getEntity()).blockFirst()))
			event.getMessage().getChannel().flatMap(c -> c.createMessage("Renewed "+ data.getEntity().getName())).subscribe();
		else
	...
	public void privacy(MessageCreateEvent event, List<String> args)
	{
		...
		data.getEntity().addRoleOverwrite(g.getEntity().getId(), data.isPrivate() ?
				PermissionOverwrite.forRole(g.getEntity().getId(), PermissionSet.none(), PermissionSet.of(Permission.VIEW_CHANNEL))
			  : PermissionOverwrite.forRole(g.getEntity().getId(), PermissionSet.of(Permission.VIEW_CHANNEL), PermissionSet.none())
		).subscribe();
		
		event.getMessage().getChannel().flatMap(c -> c.createMessage("Your channel is now "+ (data.isPrivate() ? "private" : "public"))).subscribe();
	}
	...
	public void invite(MessageCreateEvent event, List<String> args)
	{
		...
		Parser.parseMember(args.get(0), g.getEntity()).toStream().forEach(m -> Invitation.getInstance(event.getMember().get(), m));
	}
}
