...
public class WormHoleCommand extends Command {
	...
	private void linkAll() {
		...
		discordClient.getEventDispatcher().on(MessageCreateEvent.class).filter(mce ->
		id.contains(mce.getMessage().getChannelId().asLong()))
	...
	public void list(MessageCreateEvent e) {
		...
		String text = sortSetAndZipWithIndex(memory.linkedChannels.keySet())
				.map(t -> {
	...
	public void subList(MessageCreateEvent e, List<String> args) {
		...
		Optional<Tuple3<Integer, Long, Long>> foundValue = sortSetAndZipWithIndex(memory.linkedChannels.keySet()).filter(t -> t._1 == Integer.parseInt(args.get(0))).findFirst();
		if(!foundValue.isPresent())
			...
			//On trie et ajoute les index en préfixe des éléments de la liste
			String text = sortSetAndZipWithIndex(children)
					.map(t -> {
						Guild g = e.getClient().getGuildById(Snowflake.of(t._3)).block();
						return t._1 + ". #" + g.getChannelById(Snowflake.of(t._2)).block().getName() + " in **" + g.getName() + "**";
					}).collect(Collectors.joining("\n"));
	...
	private Stream<Tuple3<Integer, Long, Long>> sortSetAndZipWithIndex(Set<Tuple2<Long, Long>> set) {
		List<Tuple2<Long, Long>> temp = set.stream().sorted((t1, t2) -> (Long.toString(t1._1) + Long.toString(t1._2)).compareTo(Long.toString(t2._1) + Long.toString(t2._2))).collect(Collectors.toList());
	...
	public void link(MessageCreateEvent e, List<String> args) {
		...
		if(channels.count().block() == 2) {
			...
			if(getChildren(channel1.getId().asLong(), channel1.getGuildId().asLong()).stream().filter(t -> t._1 == channel2.getId().asLong() && t._2 == channel2.getGuildId().asLong()).count() == 0) {

				//Envoie d'un message de confirmation de lien
				String message = "Confirm link between " + channel1.getMention() + " in " + channel1.getGuild().block().getName() + " and " + channel2.getMention() + " in " + channel2.getGuild().block().getName() + "?";
				sendTemporaryValidationMessage(e.getMessage().getChannel().block(), message, Tuple.newTuple(channel1, channel2, e.getMember().get().getId()), (t, r) -> onThumbEmojiAdded(t, r));
			} else
	...
	/**
	 * Envoie un message à durée de TIME millisecondes avec deux reactions. Si la reaction "Pouce en l'air" est cliqué par l'utilisateur ayant pour Snowflake celui spécifié dans linkInfo, la méthode onValid avec pour attribut linkInfo, le message affiché et le disposable de ce dernier sera appelé. Si l'autre réaction est cliqué, supprime le message et annule le disposable.
	 * @param channel
	 * @param message
	 * @param linkInfos
	 * @param onValid
	 */
	private void sendTemporaryValidationMessage(MessageChannel channel, String message, Tuple3<GuildChannel, GuildChannel, Snowflake> linkInfos, BiConsumer<Tuple5<GuildChannel, GuildChannel, Snowflake, Message, Disposable>, ReactionAddEvent> onValid) {
		...
		Tuple5<GuildChannel, GuildChannel, Snowflake, Message, Disposable> infos = Tuple5.newTuple(linkInfos._1, linkInfos._2, linkInfos._3, m, disp);
	...
	public void remove(MessageCreateEvent e, List<String> args) {
		...
		if(!foundValue.isPresent())
			...
			Optional<Tuple3<Integer, Long, Long>> foundValue2 = sortSetAndZipWithIndex(children).filter(t -> t._1 == Integer.parseInt(args.get(1))).findFirst();
			if(!foundValue2.isPresent())
				...
				String message = "Remove link between " + channel.getMention() + " in " + guild.getName() + " and " + channel2.getMention() + " in " + guild2.getName() + "?";
				sendTemporaryValidationMessage(e.getMessage().getChannel().block(), message, Tuple.newTuple(channel, channel2, e.getMember().get().getId()), (t, r) -> onThumbEmojiAddedForRemove(t, r));
			}
		}
	}

	private void onThumbEmojiAddedForRemove(final Tuple5<GuildChannel, GuildChannel, Snowflake, Message, Disposable> infos, ReactionAddEvent e) {
		if(infos != null)
			...
			if(e.getUserId().equals(infos._3)) {
				...
				Set<Tuple2<Long,Long>> children = getChildren(infos._1.getId().asLong(), infos._1.getGuildId().asLong()).stream().filter(t -> !(t._1.equals(infos._2.getId().asLong()) && t._2.equals(infos._2.getGuildId().asLong()))).collect(Collectors.toSet());
				//On l'associe au parent
				Tuple2<Long, Long> parent = memory.linkedChannels.keySet().stream().filter(t -> t._1.equals(infos._1.getId().asLong()) && t._2.equals(infos._1.getGuildId().asLong())).findFirst().get();
				if(children.isEmpty())
	 ...
	 */
	private void onThumbEmojiAdded(final Tuple5<GuildChannel, GuildChannel, Snowflake, Message, Disposable> infos, ReactionAddEvent e) {
		if(infos != null)
			//Si l'utilisateur corresponds à celui ayant fait la requète de liaison
			if(e.getUserId().equals(infos._3)) {
				...
				if(BotUtils.returnOptional(e.getClient().getGuildById(infos._1.getGuildId())).isPresent() && BotUtils.returnOptional(e.getClient().getGuildById(infos._2.getGuildId())).isPresent()) {

					//Si les salons existent toujours au moment de la validation
					if(channelExist(infos._1.getGuild().block(), infos._1.getId().asLong()) && channelExist(infos._2.getGuild().block(), infos._2.getId().asLong())) {
						...
						Set<Tuple2<Long, Long>> values = getChildren(infos._1.getId().asLong(), infos._1.getGuildId().asLong());
						values.add(Tuple.newTuple(infos._2.getId().asLong(), infos._2.getGuildId().asLong()));
						Optional<Tuple2<Long, Long>> optional = getKey(infos._1.getId().asLong(), infos._1.getGuildId().asLong());
						memory.linkedChannels.put(optional.isPresent() ? optional.get() : Tuple.newTuple(infos._1.getId().asLong(), infos._1.getGuildId().asLong()), values);
						//Si le channel émetteur reçoit un message, on appelle la fonction copyMessage
						discordClient.getEventDispatcher().on(MessageCreateEvent.class).filter(mce ->
						infos._1.getId().asLong() == mce.getMessage().getChannelId().asLong())
						.subscribe(e2 -> copyMessage(e2));
					} else
	...
	/**
	 * Copie le message de l'événément dans tous les channels liés au salon du message
	 * @param e
	 */
	private void copyMessage(MessageCreateEvent e) {
		if(e.getMember().isPresent()) {
			...
			String message = Arrays.asList(e.getMessage().getContent().get().split(" ")).stream().map(s -> s.matches("<@\\d*>") ? e.getGuild().block().getMemberById(Snowflake.of(s.substring(2, s.length() - 1))).block().getDisplayName() : s).collect(Collectors.joining(" "));

			//On récupère tous les salons liés au channel courant
			Set<Tuple2<Long, Long>> values = getChildren(e.getMessage().getChannelId().asLong(), e.getGuildId().get().asLong());

			//On écrit le message dans les salons en question en prenant l'apparence de l'utilisateur à l'origine du message
			values.forEach(channel -> {
				if(BotUtils.returnOptional(e.getClient().getGuildById(Snowflake.of(channel._2)).block().getChannelById(Snowflake.of(channel._1))).isPresent() && e.getMessage().getContent().isPresent())
					BotUtils.sendMessageWithIdentity(e.getMember().get(), (MessageChannel) e.getClient().getGuildById(Snowflake.of(channel._2)).block().getChannelById(Snowflake.of(channel._1)).block(), message);
			});
		}
	}
	...
	private Set<Tuple2<Long, Long>> getChildren(Long channelId, Long guildId) {
		Set<Tuple2<Long, Long>> children = memory.linkedChannels.getOrDefault(memory.linkedChannels.keySet().stream().filter(t -> t.equals(Tuple.newTuple(channelId, guildId))).findFirst().orElse(Tuple.newTuple(0L, 0L)), new HashSet<>());
		Set<Tuple2<Long, Long>> childrenFiltred = children.stream().filter(t -> BotUtils.returnOptional(discordClient.getGuildById(Snowflake.of(t._2))).isPresent() && channelExist(discordClient.getGuildById(Snowflake.of(t._2)).block(), t._1)).collect(Collectors.toSet());
	 ...
	 * @param channelId
	 * @param guildId
	 * @return la clé de la map associée au channel et à la guild donnée, null si celle-ci n'existe pas
	 */
	private Optional<Tuple2<Long, Long>> getKey(Long channelId, Long guildId) {
		int isValid = 0;
		Optional<Tuple2<Long, Long>> value = memory.linkedChannels.keySet().stream().filter(t -> t.equals(Tuple.newTuple(channelId, guildId))).findFirst();
		isValid = value.isPresent() ? 0 : 1;
		if(isValid == 0)
			isValid = BotUtils.returnOptional(discordClient.getGuildById(Snowflake.of(value.get()._2))).isPresent() && channelExist(discordClient.getGuildById(Snowflake.of(value.get()._2)).block(), value.get()._1) ? 0 : 2;
		if(isValid > 0) {
			if(isValid == 2)
	 ...
	 * @param guild
	 * @param channelId
	 * @return true si un channel ayant channelId pour ID existe dans guild. False sinon
	 */
