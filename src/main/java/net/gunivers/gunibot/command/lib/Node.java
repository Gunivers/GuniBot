...
//		Command.commands.values().stream().filter(c -> c instanceof CookieCommand).findFirst().get().apply(cmd);
	...
	public Tuple2<Tuple2<List<String>, Method>, CommandSyntaxError> matches(String[] s) {
		...
		if(s.length == 1 && run != null)
			return Tuple.newTuple(Tuple.newTuple(keepValue ? new LinkedList<>(Arrays.asList(s[0])) : new LinkedList<>(), run), null);

		
		//Cas intermédiaires
		
		Tuple2<Tuple2<List<String>, Method>, CommandSyntaxError> valid = null;
		Tuple2<Tuple2<List<String>, Method>, CommandSyntaxError> farthest = Tuple2.newTuple(null, new CommandSyntaxError());
		
		for(Node n : children) {
			//Récursivité
			Tuple2<Tuple2<List<String>, Method>, CommandSyntaxError> res = n.matches(Arrays.copyOfRange(s, 1, s.length));
			
			//Si aucun n'élément n'a encore été validé et que l'élément fils courant est valide, on le garde de côté
			if(res._2 == null && valid == null)
				valid = res;
			//Sinon, si l'élément fils courant a pu aller plus loin dans la récursive que les autres, on le garde de côté
			else
