...
/**
 * Créer pour chaque objet discord. Permet de gérer et de sauvegardé des
 * données/variables grâce aux fonctions {@link #save()} et
 * {@link #load(JSONObject)}.
 * 
 * @author Syl2010
 * 
...
public abstract class DataObject<E extends Entity> {
    ...
    /**
     * Récupère les données encapsulés enregistré avec cette clef, ou les données
     * par défauts si cette clef n'existe pas
     * 
     * @param key         la clef des données à récupérés
     * @param defaultJson les données par défaut si la clef n'existe pas
     * @return les données encapsulés, ou par défaut si inexistant
     */
    public JSONObject unwrapOrDefaultDatas(String key, JSONObject defaultJson) {
	return wrappedDatas.getOrDefault(key, defaultJson);
    }

    /**
     * Fonction de sauvegarde de donnée appelé par DataCenter. Les données json sont
     * récupérés pour être enregistré dans la base de donnée. ATTENTION : Pour la
     * surcharge : super.save() doit être appelés et retransmit afin que le système
     * de "Wrapped Datas" fonctionne !
     * 
     ...
     * la base de donnée sont transmises à cette fonction. ATTENTION : Pour la
     * surcharge : super.load(json) doit être appelés avec les données json
     * d'origine afin que le système de "Wrapped Datas" fonctionne !
     * 
     * @param json les données json de la base de donnée.
     */
    public void load(JSONObject json) {
	JSONObject jsonWrappedDatas = json.optJSONObject("wrapped_datas");
	if (jsonWrappedDatas != null) {
	    for (String internalKey : jsonWrappedDatas.keySet()) {
		wrappedDatas.put(internalKey, jsonWrappedDatas.getJSONObject(internalKey));
	    }
	}
    }
	...
	if (this.getEntity() == null)
	    return "NO_DATA";
	return this.getEntity().toString().substring(this.getEntity().toString().indexOf('{')) + ": "
